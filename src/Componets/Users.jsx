  ...
  return (
    <div className='users overflow-y-auto h-[90vh] w-[500px] p-6 bg-gray-900 rounded-xl border-2 border-blue-200'>
      {data.length === 0 ? (
        ...
        <div>
          <h1 className='text-center text-3xl text-blue-200 mb-4 pb-4 border-b border-b-zinc-400'>Registered Users</h1>
          {data.map((e) => (
            <div key={e.userId} className="flex justify-between items-center mb-4 px-7 py-4 bg-zinc-950 border-[1px] border-blue-200 rounded-xl">
              ...
              <span
                onClick={() => deleteHandler(e.userId)}
                className='text-md cursor-pointer font-thin text-red-600 leading-tight'
              >
                Delete
              </span>
            </div>
